...
class FaqsModelQuestions extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		if (JLanguageAssociations::isEnabled())
		{
			$query->select('COUNT(asso2.id)>1 as association')
				->join('LEFT', '#__associations AS asso ON asso.id = a.id AND asso.context=' . $db->quote('com_faqs.item'))
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.question LIKE ' . $search . ' OR a.alias LIKE ' . $search . ')');
			}
		}
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					...
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_faqs.question')
				);
		}

		// Add the list ordering clause.
		$orderCol  = $this->state->get('list.ordering', 'a.question');
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			...
			->join('INNER', $db->quoteName('#__faqs') . ' AS c ON c.created_by = u.id')
	...
	public function getItems()
	{
		...
		if (JFactory::getApplication()->isSite())
		{
			...
			{
				// Check the access level. Remove questions the user should not see.
