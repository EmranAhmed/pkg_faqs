...
class FaqsHelperQuery
{
	 ...
	 *
	 * @param   JRegistry  $params  An options object for the question.
	 *
	...
	public static function buildVotingQuery($params = null)
	{
		if (!$params)
		{
			$params = JComponentHelper::getParams('com_faqs');
		}
		...
		if ($voting)
		{
			...
			$join   = ' LEFT JOIN #__faqs_rating AS v ON a.id = v.question_id';
		}
		else
		{
			$select = '';
	...
	/**
	 * Method to order the intro questions array for ordering
	 * down the columns instead of across.
	 * The layout always lays the introtext questions out across columns.
	 * Array is reordered so that, when questions are displayed in index order
	 * across columns in the layout, the result is that the
	 * desired question ordering is achieved down the columns.
	 *
	 * @param   array    &$questions  Array of intro text questions.
	 ...
	 * @since   3.2
	 */
	public static function orderDownColumns(& $questions, $numColumns = 1)
	{
		$count = count($questions);

		// Just return the same array if there is nothing to change.
		if ($numColumns == 1 || !is_array($questions) || $count <= $numColumns)
		{
			$return = $questions;
		}
		// We need to re-order the intro questions array.
		else
		{
			...
			// Layout the questions in column order, skipping empty cells.
			$i = 0;
			...
			// Now read the $index back row by row to get questions in right row/col
			...
			for ($row = 1; ($row <= $maxRows) && ($i < $count); $row++)
			{
				for ($col = 1; ($col <= $numColumns) && ($i < $count); $col++)
				{
					$return[$keys[$i]] = $questions[$index[$row][$col]];
